...
def load_topic_labels(topic_info, labels_path):
    ...
    return labels[["Topic", "topic_label"]]
...
def main() -> None:
    ...
    topic_totals = (
        docs.groupby(["topic", "topic_label"], dropna=False)["analysis_weight"]
        .sum()
        .rename("weighted_articles")
        .reset_index()
        .sort_values("weighted_articles", ascending=False)
    )
    top_topics = topic_totals.head(args.top_n)["topic"].tolist()
    docs_top = docs[docs["topic"].isin(top_topics)].copy()

    topic_totals.to_csv(output_dir / "topic_weighted_totals.csv", index=False)

    country_topic = weighted_group_share(docs_top, ["country", "topic_label"], "analysis_weight")
    fig_country = px.imshow(
        country_topic.pivot(index="country", columns="topic_label", values="share").fillna(0),
        aspect="auto",
    ...
    time_topic = weighted_group_share(docs_top, ["period", "topic_label"], "analysis_weight")
    fig_time = px.area(
        ...
        labels={"period": args.time_unit.title(), "share": "Topic share", "topic_label": "Topic"},
    )
    fig_time.update_layout(height=650, hovermode="x unified")
    fig_time.write_html(output_dir / "topic_shares_over_time.html")

    country_time = (
        docs_top.groupby(["country", "period", "topic_label"], dropna=False)["analysis_weight"]
        .sum()
        .rename("weighted_articles")
        .reset_index()
    )
    fig_country_time = px.line(
        ...
        labels={"period": args.time_unit.title(), "weighted_articles": "Weighted articles", "topic_label": "Topic"},
        height=1400,
    )
